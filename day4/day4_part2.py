...
for guard_key in guard_list.keys():
    if current_guard is None:
        ...
        new_sleep_count, new_sleep_minute = guard_list[guard_key].get_most_slept_minute()
        current_sleep_count, current_sleep_minute = current_guard.get_most_slept_minute()
        if current_sleep_count < new_sleep_count:
            current_guard = guard_list[guard_key]

print(f"Award for most consistently sleepy guard goes to #{current_guard.get_id()} who slept"
      f" most often at minute {current_guard.get_most_slept_minute()[1]}"
      f" for {current_guard.get_most_slept_minute()[0]} times")
print(f"That makes this part's answer {current_guard.get_id() * current_guard.get_most_slept_minute()[1]}")
